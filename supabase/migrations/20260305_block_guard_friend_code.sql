-- ============================================
-- Guard: add_friend_by_code — reject if blocked
-- Mar 5 2026
-- ============================================
-- If either user has blocked the other, the friend request is rejected.
...
BEGIN
    ...
    IF EXISTS (
        ...
        WHERE (user_id = current_user_id AND blocked_user_id = target_user_id)
           OR (user_id = target_user_id AND blocked_user_id = current_user_id)
    ) THEN
        RETURN QUERY SELECT FALSE, 'Unable to add this user as a friend'::TEXT, NULL::UUID;
        RETURN;
    END IF;
