...
| 83 | 20260417000005_email_verification_codes_add_ip.sql | 2026-04-17 | Add `ip_address` column + IP-rate-limit index to `email_verification_codes`. **Prerequisite for `email-signup` edge function deploy** — function will 500 on INSERT if the column is missing. Idempotent (`ADD COLUMN IF NOT EXISTS` / `CREATE INDEX IF NOT EXISTS`). An earlier version of this migration also added a `flow` column for verify-side cross-check; dropped 2026-04-19 as belt-and-suspenders with negligible security benefit. | LOCAL_ONLY | ADDITIVE |
| 84 | 20260418000001_drop_dead_columns.sql | 2026-04-18 | Drop 8 dead columns after onboarding simplification v2: `user_profiles.{non_negotiables, matchmaking_only, location, latitude, longitude, hometown, profile_photo_path}` + `user_preferences.looking_for`. Zero code readers/writers after cleanup in same PR. **Prod replay requires coordinated frontend + edge function deploy (all SELECTs of these columns removed in same PR).** See `docs/plans/onboarding-simplification-v2.md`. | LOCAL_ONLY | DESTRUCTIVE |
| 85 | 20260419000001_get_user_by_email_add_has_profile.sql | 2026-04-19 | DROP + CREATE `get_user_by_email` to add a `has_profile BOOLEAN` return column. Lets `email-signup` distinguish "no profile row at all" from "profile row with `profile_completed=false`". Prerequisite for **Rule B** signup-block change in `email-signup/index.ts` (blocks signup whenever a profile row exists, not just when complete — prevents accidental overwrite if a completed user retroactively trips back to `profile_completed=false`). Idempotent via `DROP FUNCTION IF EXISTS`. | LOCAL_ONLY | ADDITIVE |
